// Convert any timestamp to SGT (UTC+8) formatted string
// new Date() correctly parses both UTC ("Z") and offset ("+08:00") timestamps
function toSGT(ts) {
  const d = new Date(ts);
  const sgt = new Date(d.getTime() + 8 * 60 * 60 * 1000);
  // Format as YYYY-MM-DDTHH:mm:ss+08:00
  const iso = sgt.toISOString(); // this is in UTC, but shifted by +8
...
export async function fetchNea() {
  ...
  return Object.entries(NEA_REGIONS).map(([region, coords]) => ({
    source: "nea",
    station: region,
    timestamp: toSGT(psiItem.timestamp),
...
export async function fetchPurpleAir() {
  ...
  return json.data.map((row) => {
    ...
    return {
      ...
      timestamp: toSGT(lastSeen * 1000),
      pm25_realtime: row[idx("pm2.5")] ?? null,
